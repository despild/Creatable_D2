...
static void lcd_maintenance_menu();
...
static void lcd_filament_pla()
{
    ...
    lcd_implementation_draw_string(0, "     = PLA =");
    ...
    lcd_implementation_draw_string(3, "    Click to OK");
...
static void lcd_filament_abs()
{
    ...
    lcd_implementation_draw_string(0, "     = ABS =");
...
static void forward_feeding()
{
    ...
    {
        calculate_delta(current_position);
        plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], (n+1)*FILAMENT_UNLOAD_LEN/6, FILAMENT_LOAD_SPEED, active_extruder);
    }
...
static void reverse_feeding()
{
    ...
    for(uint8_t n=0;n<6;n++) {
        calculate_delta(current_position);
        plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], (n+1)*-FILAMENT_UNLOAD_LEN/6, FILAMENT_UNLOAD_SPEED, active_extruder);
    }
...
static void filament_load()
{
    ...
    while(degHotend(active_extruder) < EXTRUDE_MINTEMP){
        lcd_implementation_draw_string(0, "  Heating nozzle...");

        sprintf(buffer,"    %d / %d", (int)degHotend(active_extruder), EXTRUDE_MINTEMP);
        lcd_implementation_draw_string(1, buffer);
        lcd_implementation_draw_string(3, "  Click to Cancel");
    ...
    lcd_implementation_draw_string(0, "  Forward material");
...
{
    START_MENU();
    MENU_ITEM(back, MSG_CONTROL, lcd_maintenance_menu);
    ...
    switch(step) {
        ...
        case 1:
            if (current_position[Z_AXIS] == Z_MAX_POS) {
                lcd.setCursor(0, 1);
                lcd_printPGM(PSTR("Move to 1st position"));

                current_position[X_AXIS] = LEFT_PROBE_BED_POSITION;
                current_position[Y_AXIS] = BACK_PROBE_BED_POSITION;
                current_position[Z_AXIS] = Z_RAISE_BEFORE_PROBING;

                calculate_delta(current_position);
                plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], manual_feedrate[Z_AXIS]/4, active_extruder);
                st_synchronize();
        ...
        case 2:
            lcd.clear();
            lcd.setCursor(0, 1);
            lcd_printPGM(PSTR("Click to Continue"));
            if (LCD_CLICKED)
            {
                lcd_quick_feedback();
        ...
        case 3:
            // encoder user input
            lcd.setCursor(0, 0);
            lcd_printPGM(PSTR("Rotate the button"));
            ...
            if (LCD_CLICKED) {
                lcd_quick_feedback();
                // save
                zoffset[0] = current_position[Z_AXIS];
                encoderPosition = 0;
                step = 4;
                lcd_update();
            }
            break;
        // move to 2st position
        case 4:
                lcd.setCursor(0, 1);
                lcd_printPGM(PSTR("Move to 2nd position"));
        ...
        case 6:
            ...
            if (LCD_CLICKED) {
                lcd_quick_feedback();
                zoffset[1] = current_position[Z_AXIS];
                encoderPosition = 0;
                step = 7;
            }
            break;
        ...
        case 7:
                lcd.setCursor(0, 1);
                lcd_printPGM(PSTR("Move to 3rd position"));
        ...
        case 9:
            ...
            if (LCD_CLICKED) {
                lcd_quick_feedback();
                zoffset[2] = current_position[Z_AXIS];
                encoderPosition = 0;
                step = 10;
            }
            break;
...
static void lcd_maintenance_runtime_stats()
{
    ...
    sprintf(buffer, "    On-time %lu:%lu", lifetime_minutes / 60, lifetime_minutes % 60);
    lcd_implementation_draw_string(1, buffer);

    sprintf(buffer, "   Printing %lu:%lu", lifetime_print_minutes / 60, lifetime_print_minutes % 60);
    lcd_implementation_draw_string(2, buffer);
...
static void lcd_filament_menu()
{
    ...
    #endif
#endif
    MENU_ITEM(submenu, "Select PLA", lcd_filament_pla);
    MENU_ITEM(submenu, "Select ABS", lcd_filament_abs);

    MENU_ITEM(submenu, "Unload flament", filament_unload);
...
// TODO
    ...
    MENU_ITEM(submenu, "Runtime stats", lcd_maintenance_runtime_stats);
    MENU_ITEM(submenu, "Version", lcd_maintenance_version);
    ...
    MENU_ITEM(function, "Reset to default", Config_ResetDefault);
